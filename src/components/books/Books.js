...
function Books() {
  ...
  if (!isLoading && !isError && books?.length > 0) {
    ...
    filteredBooks = filteredBooks.filter((book) =>
      book?.name?.toLowerCase().includes(search_text)
    );
        ...
        <div class="order-2 xl:-order-1">
          <div class="flex items-center justify-between mb-12">
            ...
            <div class="flex items-center space-x-4">
              <button
                class={`lws-filter-btn ${!featuredClicked && "active-filter"}`}
                onClick={() => setFeaturedClicked(false)}
              >
                All
              </button>
              <button
                class={`lws-filter-btn ${featuredClicked && "active-filter"}`}
                onClick={() => setFeaturedClicked(true)}
              >
                Featured
              </button>
            </div>
